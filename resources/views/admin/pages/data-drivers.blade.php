                ...
                <tr>
                  <td class="p3 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                    <div class="flex px-2 py-1 ml-6">
                      <div>
                        @if ($drive->foto_sim)
                          <img src="{{ asset('storage/' . $drive->foto_sim) }}"
                  ...
                  <td class="p3 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                    <h6 class="px-3 mb-0 text-sm leading-normal">{{ $drive->nama }}</h6>
                    <p class="px-3 mb-0 text-xs leading-tight text-slate-400">{{ $drive->nik }}</p>
                  </td>
                  <td class="p3 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                    <h6 class="px-3 mb-0 text-sm leading-normal">{{ $drive->alamat }}</h6>
                    <p class="px-3 mb-0 text-xs leading-tight text-slate-400">{{ $drive->telepon }}</p>
                  </td>
                  <td class="p-3 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                    <span class="text-xs font-semibold leading-tight text-slate-400">{{ $drive->tgl_lahir }}</span>
                  </td>
                  <td class="p-3 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                    <span class="text-xs font-semibold leading-tight text-slate-400">{{ $drive->user_id }}</span>
                  </td>
                  <td class="p-3 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                    <button type="button" class="text-xs font-semibold leading-tight text-slate-400 editDriver" value="{{ $drive->id }}" data-bs-toggle="modal" data-bs-target="#editDriver"> Edit </button>
                  </td>
                  <td class="p-3 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                    <button type="button" class="text-xs font-semibold leading-tight text-slate-400 viewDriver" value="{{ $drive->id }}"> Lihat </button>
                  </td>
                  <td class="p-3 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                    <form action="/data-drivers/" method="post" class="hapusDrivers">
                      @csrf
                      @method('delete')
                      <button class="text-xs font-semibold leading-tight text-slate-400 hapus-driver" type="submit" value="{{ $drive->id }}">Hapus</button>
                    </form>                  
                  </td>
                </tr>
                @endforeach
...
<div fixed-plugin>
  <a fixed-plugin-button class="bottom-7.5 right-7.5 text-xl z-990 shadow-soft-lg rounded-circle fixed cursor-pointer bg-white px-4 py-2 text-slate-700" data-bs-toggle="modal" data-bs-target="#tambahDrivers">
    <i class="py-2 pointer-events-none fa fa-plus"></i>
  </a>
</div>

<!-- Modal -->
<div class="modal fade" id="tambahDrivers" tabindex="-1" aria-labelledby="judulModal" aria-hidden="true">
          ...
          <div class="mb-3">
            <div class="row"><hr>
              ...
              <div class="col-lg-2 mt-4">
                <img src="" alt="" class="priview-sim-driver img-fluid col-sm-10">
              </div>
            </div>
          </div>
      </div>
